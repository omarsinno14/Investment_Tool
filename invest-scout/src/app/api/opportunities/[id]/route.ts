...
export async function PATCH(req: Request, context: { params: Promise<RouteParams> | RouteParams }) {
  try {
    ...
    const updated = await prisma.opportunity.update({
      ...
      data: {
        ...
        ...(body.askCurrency !== undefined ? { askCurrency: toString(body.askCurrency) || "USD" } : {}),
...
export async function DELETE(_req: Request, context: { params: Promise<RouteParams> | RouteParams }) {
  try {
    const prisma = getPrismaClient();
    if (!prisma) return NextResponse.json({ error: "Database unavailable" }, { status: 500 });
